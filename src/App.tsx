...
function ClientDashboard({client,appointments,setView}:{client:any;appointments:any[];setView:(v:View)=>void}){
  ...
  return <>
    ...
    <section className="loyalty-card">
      <div className="loyalty-head"><div><span className="loyalty-label">PONTOS FIDELIDADE</span><strong>{integer.format(points)} <small>/ {integer.format(goal)}</small></strong></div><span className="loyalty-badge">Troque por R$ 100,00</span></div>
    ...
    <button className="referral-banner" onClick={()=>setView('support')}><span>🎁</span><span><b>Programa de Indicação</b><small>Indique e ganhe +100 pontos por cliente validado</small></span><i>→</i></button>
...
function Commands(){const[rows,setRows]=useState<any[]>([]),[clients,setClients]=useState<any[]>([]),[services,setServices]=useState<any[]>([]),[products,setProducts]=useState<any[]>([]),[selected,setSelected]=useState<any>(null),[pay,setPay]=useState(false),[err,setErr]=useState('');const reload=()=>Promise.all([getCommands(),getClients(),getServices(),getProducts()]).then(([c,cl,s,p])=>{setRows(c);setClients(cl);setServices(s);setProducts(p)});useEffect(()=>{void reload()},[]);const create=async()=>{try{if(!clients[0]||!services[0]||!products[0])throw new Error('Cadastre cliente, serviço e produto antes de abrir uma comanda.');const c=await createCommand({client_id:clients[0].id});await addCommandItem({command_id:c.id,service_id:services[0].id,description:services[0].name,quantity:1,unit_price:Number(services[0].price_base||0)});await addCommandItem({command_id:c.id,product_id:products[0].id,description:products[0].name,quantity:1,unit_price:Number(products[0].price||0)});await recalculateCommand(c.id);const{error}=await supabase.rpc('transition_command_state',{p_command_id:c.id,p_next:'open'});if(error)throw error;await reload()}catch(e){setErr(e instanceof Error?e.message:'Falha ao criar comanda')}};return <><Head eyebrow="CENTRAL DE COMANDAS" title="Comandas" subtitle="NEW → OPEN → PENDING_PAYMENT → PAID → CANCELLED" action={<button className="primary-btn" onClick={create}>+ Nova comanda</button>}/>{err&&<ErrorBox text={err}/>}<div className="list">{rows.map(c=><div className="list-row" key={c.id}><div><b>{c.clients?.name||'Cliente'}</b><small>{money.format(Number(c.total||0))}</small></div><div><span className="status">{c.status}</span>{c.status==='open'&&<button className="outline-btn" onClick={()=>{setSelected(c);setPay(true)}}>Fechar / Enviar pagamento</button>}</div></div>)}</div>{!rows.length&&<Empty title="Nenhuma comanda" text="Crie uma comanda real para iniciar o fluxo."/>}{pay&&selected&&<Payment command={selected} close={()=>setPay(false)} done={()=>{setPay(false);void reload()}}/>}</>}
