...
(function(){
  ...
  async function load(){
    ...
    try{
      const [i,b]=await Promise.all([
        ...
        sb.from('bank_transactions').select('id,external_transaction_id,transaction_date,amount,direction,description,pix_e2e_id,reconciliation_status,command_payment_id').order('transaction_date',{ascending:false}).limit(500)
      ]);
  ...
  function card(commandId){
    ...
    if(!active)return `<section class=\"eddu-pix-card\" style=\"margin-top:12px;padding:14px;border:1px solid rgba(127,127,127,.22);border-radius:14px\"><strong>PIX</strong><div style=\"font-size:12px;opacity:.7;margin:5px 0 10px\">Cobrança PIX real — o QR Code só aparece após confirmação do provedor.</div><button type=\"button\" onclick=\"window.EDDU_PIX_RECONCILIATION.createPix('${esc(commandId)}')\">Gerar cobrança PIX</button></section>`;
