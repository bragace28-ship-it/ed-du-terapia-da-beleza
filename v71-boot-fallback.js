/* V71 — Emergency boot fallback. Never leaves production on a blank screen. */
(function(){
  'use strict';
  function css(){
  ...
  function mount(){
    if(document.getElementById('v16-auth'))return;
    if(document.getElementById('v71-fallback'))return;
    css();
  ...
  function sync(){const f=document.getElementById('v71-fallback');if(f&&document.getElementById('v16-auth'))f.remove();if(!document.getElementById('v16-auth')&&!document.querySelector('#root[style*="visibility: visible"]'))mount()}
  ...
  setTimeout(sync,1000);setTimeout(sync,3000);setTimeout(sync,6000);
})();
