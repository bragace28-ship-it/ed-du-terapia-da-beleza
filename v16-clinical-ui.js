/* V17.7 — UI bridge for clinical records, photos and responsibility term. */
(function(){
  ...
  wait(()=>{
    wrap('saveAnam',async function(id){
      const c=db.clients.find(x=>String(x.id)===String(id));if(!c)return;
      const date=document.getElementById('ad')?.value,time=document.getElementById('at')?.value,desc=document.getElementById('ax')?.value?.trim();
      if(!date||!time||!desc)return;
      const input=document.getElementById('af');
      const files=input?.files?.length?Array.from(input.files):[];
      const r=await window.EDDU_CLINICAL.saveRecord({client_id:c.id,record_at:date+'T'+time+':00',procedure:c.anam||'',observations:desc,anamnesis:c.anam||''});
      if(r){
        c.anams=c.anams||[];c.anams.unshift({id:r.id,date,time,desc,photos:0});
        window.EDDU_RECORDS=window.EDDU_RECORDS||[];window.EDDU_RECORDS.unshift(r);
        if(files.length&&window.EDDU_STORAGE?.uploadPhotos){
          const photos=await window.EDDU_STORAGE.uploadPhotos(c.id,r.id,files);
          const item=c.anams[0];if(item)item.photos=photos.length;
          r.client_photos=photos;
        }
