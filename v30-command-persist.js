/* V30.4 — command persistence: server is source of truth; avoid corrupting local order state after save. */
(function(){
 ...
 async function saveModal(modal){
  ...
  window.dispatchEvent(new Event('eddu:refresh-data'));return{commandId,total,items:items.length};
 }
 ...
 function bind(){if(window.__EDDU_V30)return;document.addEventListener('click',async ev=>{const b=ev.target.closest('button');if(!b||!norm(b.textContent).includes('salvar alteracoes'))return;const modal=b.closest('.modal');if(!modal)return;ev.preventDefault();ev.stopImmediatePropagation();if(b.dataset.v30busy)return;b.dataset.v30busy='1';const old=b.textContent;b.textContent='Salvando...';try{const r=await saveModal(modal);toast2('Comanda salva no servidor · '+brl(r.total),true);const close=modal.querySelector('[aria-label="Fechar"], .modal-close, .close');if(close)setTimeout(()=>close.click(),350)}catch(e){console.error('EDDU V30.4',e);toast2('Não foi possível salvar: '+(e.message||e),false)}finally{b.dataset.v30busy='';b.textContent=old}},true);window.__EDDU_V30=true}
