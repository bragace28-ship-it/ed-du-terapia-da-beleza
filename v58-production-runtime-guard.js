...
(function(){
  ...
  const realReady=()=>window.__EDDU_PRODUCTION_AUTHORITY===true && window.app?.__edduRealData===true && window.app?.__edduV53Ready===true;
  let released=false;
  ...
  function errorScreen(message){
    ...
    r.style.visibility='visible';r.style.opacity='1';r.dataset.edduRuntime='error';
    document.getElementById('eddu-runtime-retry')?.addEventListener('click',()=>location.reload());
    document.getElementById('eddu-runtime-signout')?.addEventListener('click',()=>auth()?.signOut?.());
  }
  function safeRender(){
    if(!realReady())return false;
    try{if(typeof window.safeRender==='function')window.safeRender();else if(typeof window.render==='function')window.render();}catch(e){console.error('[EDDU V58] render',e);errorScreen('Falha ao renderizar os dados reais.');return false}
    release();return true;
  }
  ...
  async function boot(){
    ...
    if(!realReady()){
      ...
      if(!p || p.active===false){
        ...
        errorScreen('O Supabase não confirmou a carga de produção dentro do tempo esperado.');
      }
    }
  }
