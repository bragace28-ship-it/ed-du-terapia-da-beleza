...
async function stripeCheckout(env: any, command: any, key: string, origin: string) {
  ...
  if (!response.ok) throw new Error(data?.error?.message || 'Stripe recusou a criação do checkout.');
  return { gateway: 'stripe', externalId: data.id, checkoutUrl: data.url, raw: data };
}

async function asaasCheckout(env: any, command: any, key: string, origin: string, method: 'PIX' | 'CREDIT_CARD') {
  ...
  const response = await fetch(`${base}/v3/checkouts`, {
    method: 'POST',
    headers: { accept: 'application/json', 'content-type': 'application/json', access_token: env.ASAAS_API_KEY, 'Idempotency-Key': key },
    body: JSON.stringify(payload),
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data?.errors?.[0]?.description || data?.message || 'Asaas recusou a criação do checkout.');
...
async function pagbankCheckout(env: any, command: any, key: string, origin: string, method: 'PIX' | 'CREDIT_CARD') {
  ...
  const response = await fetch(`${base}/checkouts`, {
    method: 'POST',
    headers: { accept: 'application/json', 'content-type': 'application/json', Authorization: `Bearer ${env.PAGBANK_ACCESS_TOKEN}`, 'x-idempotency-key': key },
    body: JSON.stringify(payload),
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data?.error_messages?.[0]?.description || data?.message || 'PagBank recusou a criação do checkout.');
...
export async function onRequestPost({ request, env }: any) {
  try {
    ...
    const { profile, userId } = await requireUser(request, sql);
    ...
    const rows = await sql`select c.id,c.total,c.status,c.client_id,c.created_by,c.payment_gateway,c.payment_installments,cl.name,cl.email from public.commands c left join public.clients cl on cl.id=c.client_id where c.id=${commandId} limit 1`;
    ...
    const key = request.headers.get('idempotency-key') || `${command.id}:${gateway}:${method}`;
    const origin = originOf(request);
    const result = gateway === 'stripe'
      ? await stripeCheckout(env, command, key, origin)
      : gateway === 'asaas'
        ? await asaasCheckout(env, command, key, origin, method === 'pix' ? 'PIX' : 'CREDIT_CARD')
        : await pagbankCheckout(env, command, key, origin, method === 'pix' ? 'PIX' : 'CREDIT_CARD');
    const pm = await sql`select id from public.payment_methods where active=true and code in (${method === 'pix' ? 'pix' : 'credit_card'},${method === 'pix' ? 'PIX' : 'card'}) limit 1`;
    const commandPayment = await sql`insert into public.command_payments (command_id,payment_method_id,gateway,gross_amount,amount,fee_amount,net_amount,installments,external_transaction_id,status,metadata,created_by) values (${command.id},${pm[0]?.id || null},${result.gateway},${Number(command.total)},${Number(command.total)},0,${Number(command.total)},${Number(command.payment_installments || 1)},${result.externalId},'PENDING',${JSON.stringify({ checkout_url: result.checkoutUrl, method, idempotency_key: key })}::jsonb,${userId}) returning id`;
